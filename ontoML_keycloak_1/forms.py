...
from .models import User, Item, Query
...
class SPARQLQueryForm(FlaskForm):
    def validate_namespace(self, namespace_to_check):
        query = Query.query.filter_by(namespace=namespace_to_check.data).first()
        if query:
            raise ValidationError("Namespace already exists! Please confirm the spelling or check the data model")
